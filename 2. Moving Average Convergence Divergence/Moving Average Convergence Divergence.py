...
def MACD(start):
    global long
    global short
    macd_long = EMA(start,long)
    macd_short = EMA(start, short)

    ma =  macd_short-macd_long
    return ma


for day in range(400,549,1):
    now_value = MACD(day)
    pre_value = MACD(day-1)

    if pre_value<=0 and now_value>0:
        profit = (originaldata[day+2][0]- originaldata[day+1][0])*50
